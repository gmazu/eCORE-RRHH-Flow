...
class PanelA_DistribucionDefinida(Scene):
    def construct(self):
        ...
        try:
            ...
            distribucion_definida = sistema.calcular_distribucion_definida()
        ...
        for i, (zona_nombre, cantidad) in enumerate(distribucion_definida.items()):
        ...
        if not zonas_data:
            self.mostrar_error("No hay zonas definidas en configuración")
            return
        
        # ========================================
        # TÍTULO DEL PANEL
        # ========================================
        titulo = Text("DISTRIBUCIÓN DEFINIDA", font_size=36, color=WHITE)
        subtitulo = Text("Configuración Planificada", font_size=20, color=GRAY)
        ...
        max_personas = max([z["personas"] for z in zonas_data])
        bar_width_scale = 5.0
        bar_height = 0.5
        spacing = 0.9
        ...
        # ========================================
        self.play(
            LaggedStart(*[FadeIn(label) for label in labels_group], lag_ratio=0.1)
        )
        self.play(
            LaggedStart(*[GrowFromEdge(barra, LEFT) for barra in barras_group], lag_ratio=0.15)
        )
        self.play(
            LaggedStart(*[FadeIn(valor) for valor in valores_group], lag_ratio=0.1)
        )
        
        # ========================================
        # TOTAL PLANIFICADO
        # ========================================
        total = sum([z["personas"] for z in zonas_data])
        linea_separadora = Line(LEFT * 6, RIGHT * 6, color=GRAY).shift(DOWN * (inicio_y - num_zonas * spacing + 0.5))
        total_text = Text(f"TOTAL PLANIFICADO: {total} personas", font_size=26, color=WHITE)
        total_text.next_to(linea_separadora, DOWN, buff=0.3)
        ...
        # INDICADOR DE ESTADO
        # ========================================
        estado = Text("BASELINE OPERATIVO", font_size=18, color="#4A90E2", slant=ITALIC)
